  ...
  <div class="dashboard-container">
    <div class="work-block">
      <el-card class="header-box-card">
        <div slot="header" class="workplace clearfix">
    ...
    <div class="main-block">
      ...
      <div class="index-item">
        <el-card class="main-box-card">
          ...
          <div v-for="(item,idx) in itemList" :key="idx" class="text item">
            <div>
              <span class="svg-container">
            ...
            <div>
              <span class="item-status">{{item.isFinish==true?"已完成":"未完成"}}</span>
            </div>
          </div>
        </el-card>
      </div>
        ...
        <el-card class="main-box-card">
          ...
          <div v-for="(item,idx) in offWorkList" :key="idx" class="text item">
            <div>
              ...
              <span class="title" v-text="item.offReason" />
            </div>
            <div>
              <span>{{item.allow==null?"未审核":(item.allow==true?"审核通过":"未批准")}}</span>
            </div>
          </div>
        </el-card>
      </div>
      ...
      <div class="index-item">
        <el-card class="main-box-card">
          ...
          <vue-seamless-scroll
            :data="noticeList"
            style="overflow: hidden; margin-top: -14px;"
          ...
          >
            <ul class="notice-ul">
              <li v-for="(item,idx) in noticeList" :key="idx">
                ...
                <span class="date" v-text="item.createTime" />
              </li>
            </ul>
          </vue-seamless-scroll>
        </el-card>
    ...
    <!-- 打卡 -->
    <el-dialog center title="打卡" :visible.sync="clockDialog" :before-close="handleClockClose">
      <el-row :gutter="20">
        <el-col :span="16">
...
import { getOffWorkList } from "@/api/offWork";
...
export default {
  ...
  mounted() {
    //通知
    getNoticeList(this.userId).then((resp) => {
      this.noticeList = resp.data;
    });
    getOffWorkList(this.userId).then((resp) => {
      this.offWorkList = resp.data;
    });
    getItemList(this.userId).then((resp) => {
